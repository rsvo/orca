...
//
// Those numbers are the `!useConptyDll` branch, which is the branch a RELAY runs. The desktop sets
// `useConptyDll: true` and takes the other branch, where upstream already destroys the input socket,
// so the desktop never executes the hunk these numbers describe. The divergence pinned below is
// about which branch each host runs -- not about a regression in the shipped desktop app.
...
describe('Windows SSH relay node-pty ConPTY teardown patch', () => {
  ...
  // teardown partway. The desktop takes the other branch and is unaffected either way; what this
  // guards is a patch sync putting the early placement onto the relay's branch.
